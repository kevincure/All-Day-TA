...
# the "m: " and "s: " details for generating multiple-choice and short-answer questions are still not properly developed
# they too often give answers which are not the ones in our text.  M is better for now.
...
# Allow storage of concepts to understand how Q&A from pre-selected menu
# Allow video/audio of class to be auto-handled
# Add slide descriptions
# Finish front-end for uploading docs and changing settings
# Once GPT_4 API, check first if it's a math question then send to GPT_4, also the followup part will work better
